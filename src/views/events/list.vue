    ...
    <div class="v-list" v-loading="loading" element-loading-text="加载中">
        <el-card class="box-card">
            <div slot="header" class="clearfix">
                ...
                <!-- 搜索 -->
        <div class="form-search"><!--搜索条件栏-->
            <el-form @submit.prevent.stop :inline="true" size="mini">
                                <el-form-item label="活动标题">
                
                    <el-input v-model="search.eventName"></el-input> 
                    
                </el-form-item>                <el-form-item label="活动状态">
                
                    <el-input v-model="search.eventStatus"></el-input>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row >
            ...
            <el-table-column label="活动容量"width="80">
                <template slot-scope="{row}">
                    {{  row.maxJoin  }}                </template>
            </el-table-column>
            <el-table-column label="已报名人数"width="80">
                <template slot-scope="{row}">
                    {{  row.reginum  }}                </template>
            </el-table-column>
            <el-table-column label="报名成功人数"width="80">
                <template slot-scope="{row}">
                    {{  row.success  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/eventsdetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/eventsupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.events.list , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.events.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
