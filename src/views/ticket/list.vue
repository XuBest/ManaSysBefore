    ...
    <div class="v-list" v-loading="loading" element-loading-text="加载中">
        ...
        <el-card class="box-card">
            ...
            >
                ...
                <el-table-column label="票务编号" width="130">
                    <template slot-scope="{row}">
                        {{ row.ticketkey }}
                    </template>
                </el-table-column>
                <el-table-column label="票务名称">
                    <template slot-scope="{row}">
                        {{ row.ticketname }}
                    </template>
                </el-table-column>
                <el-table-column label="图片" width="100">
                    <template slot-scope="{row}">
                        <e-img :src="row.picture" type="list" style="max-width:120px"></e-img>
                    </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            ...
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/ticketdetail',query:{id:row.id } })"
                                           icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit"
                                           @click="$goto({path:'/admin/ticketupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.ticket.list, filter).then(res => {
                    this.loading = false;
            ...
            // 删除某行方法
            deleteItem(row) {
                this.$confirm('确定删除？', '提示', { // 弹出 确认框提示是否要删除
                    type: 'warning'
                }).then(() => {// 确定操作

                    this.loading = true; // 滚动条
                    this.$post(api.ticket.delete, {// 提交后台
                        id: row.id
                    }).then(res => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
